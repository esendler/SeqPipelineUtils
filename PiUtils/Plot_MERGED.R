...
names_ind<-readLines("names_ind.txt")
setwd("bams")
...
##   for each sample with multiple ...L1, ...L2,    ...L3   runs
...
reads_ind<-reads
reads_ind$Filename<-gsub("_L.*","",reads_ind$Filename)
reads<-aggregate(. ~ Filename, data=reads_ind, FUN=sum)
...
pdf("Merged_counts.pdf",width=12,height=16)
...
pdf("MERGED-Percent Clean Mapped.pdf")
